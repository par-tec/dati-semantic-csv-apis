...
def test_collect_data(tmp_path: Path):
    ...
    vocabulary_ttl = tmp_path / f"{data['key_concept']}.ttl"
...
def test_add_data_to_db(tmp_path: Path):
    ...
    with sqlite3.connect(_sqlite_path(SQLITE_URL)) as conn:
        ...
        vocabulary_df: pd.DataFrame = pd.read_sql(
            f'SELECT id, url, _text FROM "{data["vocabulary_uri"]}" LIMIT 5',
            conn,
        )
    ...
    for _, row in vocabulary_df.iterrows():
        assert row["id"] is not None, "Row id is missing"
        assert row["url"] is not None, "Row url is missing"
        assert row["_text"] is not None, "Serialized row payload is missing"
...
def add_data_to_db(folder: Path, db_url: str, data: dict[str, str]):
    ...
    openapi_path = folder / f"{key_concept}.oas3.yaml"
    ...
    with sqlite3.connect(sqlite_path) as conn:
        ...
        pd.DataFrame(rows).to_sql(
            vocabulary_uri, conn, if_exists="replace", index=False
        )
